...
node.jsを起動していた時と違い、表示は自動で行われません。次の方法で表示します。
...
関数の呼び出しは変数の後ろに`()`をつけます。引数がある場合は`()`内に`,`区切りで指定します。
...
配列にたいして次のようにすることで、繰り返し処理が使えます。（次の例以外にも書き方は沢山あります）
